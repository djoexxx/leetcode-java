...
public class StringToIntegerAtoi {
  public int myAtoi(String s) {
    ...
    // Skip leading zeros
    while (leadingZeros) {
      leadingZeros = s.charAt(count) != 0 ? false : true;
      if (leadingZeros) {
        ++count;
      } else {
        leadingZeros = false;
    ...
    while (s.length() > count) {
      ...
      converted = (converted * 10) + Character.getNumericValue(current);
      if (converted > (long) Integer.MAX_VALUE) {
        if (isPositive) {
          converted = (long) Integer.MAX_VALUE;
          break;
        } else {
          converted = (long) Integer.MIN_VALUE;
          break;
        }
      }
      ++count;
    ...
    if (isPositive) {
      finalInt = converted > (long) Integer.MAX_VALUE ? Integer.MAX_VALUE : (int) converted;
    } else {
      finalInt = -1 * converted < (long) Integer.MIN_VALUE ? Integer.MIN_VALUE : (int) (-1 * converted);
    }
